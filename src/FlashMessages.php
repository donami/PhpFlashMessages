...
class FlashMessages implements \Anax\DI\IInjectionAware {
    ...
    public function __construct()
    {
        ...
        // if (!$this->di->session->has('flash_messages')) $this->di->session->set('flash_messages', []);
    }
    ...
    public function add($message, $type=self::defaultType, $redirectUrl=null, $sticky=false)
    {
        ...
        $array = $this->di->session->get('flash_messages');

        $array[$type][] = ['sticky' => $sticky, 'message' => $message];

        $this->di->session->set('flash_messages', $array);
    ...
    public function display($types=null, $print=true)
    {
        ...
        //if ($this->di->session->has('flash_messages')) return false;
        ...
        foreach ($types as $type) {
            $data = $this->di->session->get('flash_messages');

            if (!isset($data[$type]) || empty($data[$type])) continue;

            foreach ($this->di->session->get('flash_messages') as $msgData) {
              foreach ($msgData as $key => $value) {
                $output .= $this->formatMessage($value, $type);
              }
            }
            $this->clear($type);
        }
